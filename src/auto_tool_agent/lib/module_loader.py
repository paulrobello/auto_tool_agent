...
class ModuleLoader(FileSystemEventHandler):
    ...
    def load_module(self, module_path: Path) -> None:
        ...
        try:
            if not module_path.is_file() or module_path.name.endswith("~") or "__init__" in module_path.name:
                return
            if module_path.name.endswith(".py"):
                spec = importlib.util.spec_from_file_location(module_name, module_path)
                if not spec:
                    ml_log.error("[red]Error[/red]: unable to load module spec %s", module_name)
                    return
