...
from auto_tool_agent.lib.llm_providers import LlmProvider, is_provider_api_key_set
from auto_tool_agent.lib.par_ollama_embeddings import ParOllamaEmbeddings
...
class LlmConfig:
    ...
    def _build_ollama_llm(self) -> BaseLanguageModel | BaseChatModel | Embeddings:
        ...
        if self.mode == LlmMode.EMBEDDINGS:
            return ParOllamaEmbeddings(ollama_host=ollama_host, model=self.model_name)
    ...
    def _build_bedrock_llm(self) -> BaseLanguageModel | BaseChatModel | Embeddings:
        ...
        session = boto3.Session(
            ...
            profile_name=os.environ.get("AWS_PROFILE", "vgh-paul-robello-aws-sb"),
        )
        ...
        if self.mode == LlmMode.BASE:
            return BedrockLLM(
                ...
                model=self.model_name,  # pyright: ignore [reportCallIssue]
                temperature=self.temperature,  # pyright: ignore [reportCallIssue]
            )
        if self.mode == LlmMode.CHAT:
